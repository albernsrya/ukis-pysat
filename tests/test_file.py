...
class FileTest(unittest.TestCase):
    def test_env(self):
        with self.assertRaises(KeyError,
                               msg=f"No environment variable Key found"):
            psf.env_get("Key")
    ...
    def test_get_sentinel_scene_from_path_testfiles(self):
        with psf.get_sentinel_scene_from_dir(path_testfiles) as (full_path,
                                                                 ident):
    ...
    def test_get_sentinel_scene_from_dir_joinpath(self):
        with psf.get_sentinel_scene_from_dir(
                path_testfiles.joinpath("another_scene")) as (full_path,
                                                              ident):
    ...
    def test_get_footprint_from_manifest(self):
        self.assertEqual(
            psf.get_footprint_from_manifest(
                path_testfiles.joinpath("manifest.safe")).wkt,
    ...
    def test_get_pixel_spacing(self):
        self.assertEqual(psf.get_pixel_spacing(path_testfiles),
        ...
        self.assertEqual(psf.get_pixel_spacing(str_path),
                         (40.0, 0.0003593261136478086))
