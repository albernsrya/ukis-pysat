...
class RasterTest(unittest.TestCase):
    ...
    def test_init_with_array(self):
        with Image(self.img.arr,
                   crs=self.img.dataset.crs,
                   transform=self.img.dataset.transform) as img:
            self.assertTrue(np.array_equal(self.img.arr, img.arr))
    ...
    def test_set_array(self):
        with Image(TEST_FILE, dimorder="last") as im:
            ...
            self.assertTrue(np.array_equal(im.arr,
                                           np.ones(shape=im.arr.shape)))
    ...
    def test_mask_image(self):
        with self.assertRaises(
                TypeError,
                msg="bbox must be of type tuple or Shapely Polygon"):
            self.img.mask([1, 2, 3])
    ...
    def test_warp(self):
        ...
        self.assertNotEqual(source_img.dataset.transform,
                            target_img.dataset.transform)
        source_img.warp("EPSG:3857", target_align=target_img)
        self.assertEqual(source_img.dataset.transform,
                         target_img.dataset.transform)
    ...
    def test_dn2toa(self):
        ...
        for i in range(len(tests)):
            ...
            self.assertTrue(
                np.array_equal(img_dn.arr, img_toa.arr, equal_nan=True))
            img_dn.close()
            img_toa.close()
    ...
    def test__lookup_bands(self):
        self.assertEqual(
            ["1", "2", "3"],
            self.img._lookup_bands(Platform.Landsat5,
                                   ["Blue", "Green", "Red"]),
        )
        self.assertEqual(
            ["8", "10", "11"],
            self.img._lookup_bands(Platform.Landsat8,
                                   ["PAN", "Tirs1", "Tirs2"]),
        )
    ...
    def test_get_tiles(self):
        for idx, each in enumerate(self.img.get_tiles(5, 5, 1)):
            ...
            if idx == 2578:
                self.assertEqual(
                    each,
                    windows.Window(col_off=79, row_off=649, width=7, height=7))
    ...
    def test_get_subset(self):
        for idx, each in enumerate(self.img.get_tiles(5, 5, 1)):
            if idx == 2578:
                ...
                self.assertTrue(
                    np.array_equal(array,
                                   np.zeros(shape=(7, 7), dtype=array.dtype)))
                self.assertEqual(
                    bounds,
                    (
                        11.903960582768779,
    ...
    def test_get_dask_array(self):
        self.assertIsInstance(self.img.to_dask_array(chunk_size=(1, 10, 10)),
    ...
    def test_write_to_file(self):
        ...
        self.img.write_to_file(r"result.tif",
                               np.uint8,
                               compress="packbits",
                               kwargs={"tiled": True})
        with Image("result.tif") as img2:
        ...
        with Image(self.img.arr,
                   crs=self.img.dataset.crs,
                   transform=self.img.dataset.transform) as img3:
